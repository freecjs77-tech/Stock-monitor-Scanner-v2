...
사용법:
    ...
    python quick_render.py --ai         # Groq AI 요약 포함 (GROQ_API_KEY 필요)
    python quick_render.py NVDA --open --ai
"""
...
def run(tickers=None, auto_open=False, use_ai=False):
    ...
    # AI 요약 (--ai 플래그 + GROQ_API_KEY 있을 때)
    ai_data = None
    if use_ai and generate_ai_summary:
        print(f"\n  [AI] Groq 요약 생성 중...", end=' ', flush=True)
        try:
            ai_data = generate_ai_summary(stocks)
            print("OK" if ai_data else "스킵 (키 없음)")
        except Exception as e:
            print(f"오류: {e}")
    ...
    try:
        generate_summary_page(stocks, summary_path, ai_data=ai_data)
...
if __name__ == '__main__':
    ...
    use_ai    = '--ai'   in args
    args = [a for a in args if a not in ('--open', '--ai')]

    tickers = [a.upper() for a in args] if args else None

    run(tickers=tickers, auto_open=auto_open, use_ai=use_ai)
